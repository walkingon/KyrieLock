...
- **Rust高性能加密引擎**
  ...
  - 零成本抽象，提供接近硬件的性能
- **大文件流式处理**
  - 分块加密/解密（移动端128MB，桌面端256MB块大小）
  - 流式读写，避免内存溢出
  - 支持GB级大文件的加密和解密
  - 自动块长度记录，确保精确解密
- FFI跨语言调用（Dart ↔ Rust）
- 自定义文件头格式(Magic String + 版本号 + 提示词 + 分块信息)
...
- **智能内存管理**
  - **小文件（移动端≤200MB，桌面端≤400MB）**：直接在内存中解密，零临时文件开销
    - 图片查看器：Image.memory直接加载
    - 文本查看器：String.fromCharCodes直接加载
    - 视频/音频/PDF查看器：内存写入临时文件后播放
  - **大文件（移动端>200MB，桌面端>400MB）**：流式解密到临时文件，避免内存溢出
    - 解密过程峰值内存：移动端128MB，桌面端256MB（分块大小）
    - 视频/音频/PDF查看器：直接使用临时文件，无额外内存占用
    - 图片/文本查看器：读取临时文件后立即删除
    - 支持GB级大文件在移动设备上流畅打开
  - **临时文件自动清理**：查看器关闭时自动删除所有临时文件
...
- **Rust高性能加密引擎**
  ...
  - 零成本抽象，无运行时开销
- **大文件流式处理**
  - 移动端128MB分块，桌面端256MB分块，平衡性能与UI响应性
  - 流式读写，内存占用稳定在单块大小
  - 支持1GB+大文件加密/解密，无OOM风险
  - 加密效率提升：移动端约9次调用/GB，桌面端约4次调用/GB
- **智能解密策略**
  - 小文件（移动端≤200MB，桌面端≤400MB）：内存直接解密，性能最优
  - 大文件（移动端>200MB，桌面端>400MB）：自动切换到临时文件模式，避免OOM
