# KyrieLock

一款跨平台文件加密工具。**加密器支持对任意类型的文件进行加密保护**,而内置的查看器支持视频、图片、音频、文本和PDF文件的查看。用户可以对文件设置密码加密,选择已加密的文件进行查看时,输入正确密码即可正常查看,也可以将加密文件还原为无加密文件。

## 已支持的操作系统

- Windows
- Android

## 加密功能

**加密器支持任意文件类型**,包括但不限于视频、图片、音频、文档、压缩包等。加密过程与文件类型无关,可对任意文件进行加密保护。

## 查看器支持的文件类型

内置查看器支持以下文件类型的直接查看:

- **视频**: mp4, avi, mkv, mov, wmv, flv
- **图片**: jpg, jpeg, png, gif, bmp, webp
- **音频**: mp3, wav, flac, aac, m4a, ogg
- **文本**: txt, md, json, xml, csv
- **PDF**: pdf

**注意**: 对于查看器不支持的文件类型(如Office文档等),您可以通过"解密文件"功能将其解密,然后使用对应的第三方软件打开查看。

## 核心功能

### 1. 文件关联
- **Windows**: 应用首次启动时自动注册为.kyl文件的默认打开程序
  - 双击.kyl加密文件即可自动启动应用并打开
  - 支持通过文件管理器直接打开加密文件
  - 加密文件打开时自动提示输入密码,密码正确后即可查看
- **Android**: 由于系统限制(尤其是Android 13+和部分定制ROM如ColorOS),无法直接关联.kyl文件
  - 推荐方式: 在文件管理器中**长按.kyl文件** → 选择**"分享"** → 选择本应用打开
  - 或在应用内使用"打开文件"功能手动选择.kyl文件

### 2. 打开文件
- 用户选择文件(仅限查看器支持的文件类型),系统自动检测文件是否已加密
- 若是普通文件则直接使用对应的查看器打开
- 若是加密文件,提示输入密码(若有提示词则显示),密码正确后解密并使用对应查看器打开
- 若选择的文件类型不在查看器支持列表中,系统将提示用户使用"解密文件"功能

### 3. 加密文件
- 用户可选择单个或多个**任意类型**的文件,提示用户设置密码,以及可选填写的提示词
- 系统将文件逐一加密并导出加密后的文件
- 批量加密时,已加密的文件自动跳过,避免重复加密
- 加密过程显示实时进度,包括当前进度、文件总数和正在处理的文件名
- 用户选择文件保存路径,文件自动重命名新增".kyl"扩展名
  - 如原文件名为"example.mp4",则加密后保存的文件名为"example.mp4.kyl"
- 加密文件时在文件头写入自定义的独特字符串(Magic String: KYRIE_LOCK),以区分是否为加密文件
- 文件头还包含版本号信息,便于后续升级功能时做兼容
- 版本号后为可选的提示词信息,解密时显示以帮助用户回忆密码

### 4. 解密文件
- 用户可选择**任意类型**的文件,系统读取文件头判断是否包含Magic String
- 若是加密文件,提示用户输入密码(若有提示词则显示)
- 验证密码正确后,将文件解密并让用户选择保存路径
- 解密过程显示进度信息,包括当前处理的文件名
- 文件重命名时若包含.kyl扩展名则自动去除
- 解密后的文件可使用对应的第三方软件打开查看

## 技术架构

项目使用Flutter统一开发，采用分层架构设计:

### 加密/解密层
- **文件类型无关**的二进制流处理,支持任意文件类型
- **Rust高性能加密引擎**
  - AES-256-CBC加密算法（使用Rust原生实现）
  - SHA-256密钥派生（基于RustCrypto生态）
  - 编译器优化（LTO、单代码单元、O3级别优化）
  - 零成本抽象，提供接近硬件的性能
- FFI跨语言调用（Dart ↔ Rust）
- 自定义文件头格式(Magic String + 版本号 + 提示词)
- 兼容Dart纯实现（可通过标志切换）

### 查看器层
- **文件类型感知**的展示逻辑,仅支持特定文件类型
- 根据文件扩展名自动选择合适的查看器
- 支持加密文件的透明解密查看
- 对不支持的文件类型提示用户通过解密功能处理
- **智能内存管理**
  - 图片查看器：直接从内存加载解密数据(Image.memory)
  - 文本查看器：直接从内存加载解密数据(String.fromCharCodes)
  - 视频/音频/PDF查看器：使用临时文件(受限于第三方库API),在查看器关闭时自动清理

### 平台集成层
- **Windows文件关联**
  - 通过Windows Registry API注册.kyl文件扩展名
  - 创建ProgID和文件类型关联
  - 支持命令行参数接收文件路径
  - 使用MethodChannel与Dart层通信
- **Android分享集成**
  - 通过AndroidManifest.xml配置ACTION_SEND intent-filter
  - 支持从文件管理器分享文件到应用
  - 处理content URI并自动转换为可访问的文件路径
  - Kotlin原生代码与Flutter通信
  - **注意**: 由于Android 13+及部分定制ROM(如ColorOS)的限制,不支持直接双击打开.kyl文件,需使用分享方式

## 安全特性

- AES-256-CBC加密标准,确保文件安全
- SHA-256密钥派生,防止暴力破解
- **Rust内存安全保证**：防止缓冲区溢出、空指针等安全漏洞
- **临时文件自动清理**：查看加密文件时创建的临时文件在查看器关闭时自动删除
  - 视频播放器：在dispose时自动删除临时视频文件
  - 音频播放器：在dispose时自动删除临时音频文件
  - PDF查看器：在dispose时自动删除临时PDF文件
  - 图片/文本查看器：直接从内存加载,无需临时文件
- 密码提示词帮助用户记忆密码
- 版本控制支持未来的兼容性扩展

## 性能优化

- **Rust高性能加密引擎**
  - 原生性能，比纯Dart实现显著提升
  - 编译期优化：LTO（链接时优化）
  - 零成本抽象，无运行时开销
- 支持切换Dart/Rust实现
- 内存优化的FFI调用

## 开发路线

- [x] 核心加密功能开发
  - [x] 实现加密算法核心模块(AES-256加密,SHA-256密钥派生)
  - [x] 实现文件加密功能(包含Magic String: KYRIE_LOCK和版本号)
  - [x] 实现文件解密功能(支持密码验证和版本兼容)
- [x] 文件查看器开发
  - [x] 实现视频播放器(支持普通视频和加密视频解密播放)
  - [x] 实现图片查看器(支持缩放、平移)
  - [x] 实现音频播放器(支持播放控制)
  - [x] 实现文本查看器(支持滚动、选择)
  - [x] 实现PDF查看器(支持翻页)
- [x] 用户界面开发
  - [x] 主页菜单
  - [x] 文件选择器
  - [x] 密码输入对话框
- [x] 集成测试并成功构建Windows平台应用程序
- [x] 批量加密功能
  - [x] 支持选择多个文件进行批量加密
  - [x] 自动跳过已加密的文件
  - [x] 实时显示加密进度(当前进度/总数,当前文件名)
- [x] 加密/解密进度显示
  - [x] 创建通用进度对话框组件
  - [x] 加密时显示批量处理进度
  - [x] 解密时显示当前文件进度
- [x] 性能优化
  - [x] 使用Rust实现核心加密模块
  - [x] FFI集成（Dart ↔ Rust）
  - [x] 编译器优化（LTO、O3优化级别）
- [x] 文件关联功能
  - [x] Windows平台注册.kyl文件默认打开程序
  - [x] Android平台分享打开支持(通过ACTION_SEND)
  - [x] 跨平台MethodChannel通信实现
- [ ] 适配Linux系统
- [ ] 适配macOS系统
- [ ] 适配iOS系统

## 未来规划

1. **性能优化**
   - ~~大文件流式加解密~~（已通过Rust实现优化）
   - ~~内存优化~~（已通过Rust零成本抽象优化）
   - 多线程并行处理大文件

2. **功能增强**
   - ~~文件批量加密/解密~~
   - ~~文件关联(Windows)~~
   - ~~文件分享打开(Android)~~
   - 最近使用记录
   - Linux/macOS文件关联支持
   - Android直接打开.kyl文件支持(需等待系统限制解除)

## 开发环境

### 必需工具
- Flutter SDK (3.10.1+)
- Rust工具链 (1.70+)
- 对应平台的构建工具
  - Windows: Visual Studio 2022或MSVC构建工具
  - macOS: Xcode
  - Linux: GCC/Clang

### 构建步骤

1. 安装依赖
```bash
flutter pub get
```

2. 编译Rust加密库
```bash
cd rust_crypto
cargo build --release
cd ..
# 当rust有修改后，运行脚本build_android_rust.ps1为Android平台构建依赖
```

3. 运行/构建应用
```bash
# 开发模式
flutter run

# 发布构建
flutter build windows --release  # Windows
flutter build macos --release     # macOS
flutter build linux --release     # Linux
flutter build apk --release       # Android
flutter build ios --release       # iOS
```


## 许可证

本项目仅供学习和个人使用。