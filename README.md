...
- **大文件流式处理**
  ...
  - 加密速度：约16次加密调用/GB（相比10MB块提升6倍）
- **智能解密策略**
  - 小文件（≤100MB）：内存直接解密，性能最优
  - 大文件（>100MB）：自动切换到临时文件模式，避免OOM
